...
def model_run(sub, task, recorded_tr=0.610, brain_mask='/foundcog/forrestgump/mask/infants/nihpd_asym_02-05_fcgmask_2mm.nii.gz', fwd_cutoff=0.5, derivs='smoothing'):
    ...
    def _get_fnames(sub, task, derivs):
        ...
        for sesnum in [1, 2]: 
            for runnum in [1, 2,3,4]:
                ...
                event_file = join(
                    base_dir, 'foundcog-infants-2m', 'resmem_model', 'expanded_events',
    ...
    for runidx in range(len(paths['func'])):
        ...
        model_path = os.path.abspath(f'/foundcog/forrestgump/foundcog-infants-2m/resmem_model/glm/model/{sub}_task-{task}_run-{runidx + 1}_model.pkl')
        ...
        if os.path.exists(model_path):
            ...
            with open(model_path, 'rb') as f:
                ...
                plt.tight_layout()
                plt.savefig(f'/foundcog/forrestgump/foundcog-infants-2m/resmem_model/glm/confound_corr_{sub}_run_{runidx + 1}.png')
                plt.show()
            ...
            try:
                ...
                plot_path = f'/foundcog/forrestgump/foundcog-infants-2m/resmem_model/glm/design_matrices/design_matrix_{sub}_run_{runidx + 1}.png'
                ...
                vif_output_path = f'/foundcog/forrestgump/foundcog-infants-2m/resmem_model/glm/vif/vif_{sub}_run_{runidx + 1}.csv'
        ...
        for contrast_id, contrast_val in {
            ...
            try: 
                ...
                beta_map_path = os.path.abspath(f'/foundcog/forrestgump/foundcog-infants-2m/resmem_model/glm/beta_maps/{sub}_task-{task}_run-{runidx + 1}_{contrast_id}_beta_map.nii.gz')
