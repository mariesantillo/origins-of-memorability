...
def model_run(sub, task, recorded_tr=0.656, brain_mask='/foundcog/forrestgump/mask/adults/mni152_resampled_brain_mask_adult.nii.gz', fwd_cutoff=0.5, derivs='normalized_to_common_space'):
    ...
    def _get_fnames(sub, task, derivs):
        ...
        output_dir = '/foundcog/forrestgump/foundcog-adult-2/resmem_model/'
        funcpaths = []
        eventpaths = []
        motionpaths = []
    ...
    for runidx in range(len(paths['func'])):
        ...
        model_path = os.path.abspath(f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/model/{sub}_task-{task}_run-{runidx + 1}_model.pkl')
        ...
        if os.path.exists(model_path):
            ...
            try:
                ...
                vif_output_path = f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/vif_{sub}_run_{runidx + 1}.csv'
                ...
                plt.tight_layout()
                plt.savefig(f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/confound_corr_{sub}_run_{runidx + 1}.png')
                plt.show()
            ...
            try:
                ...
                plot_path = f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/design_matrices/design_matrix_{sub}_run_{runidx + 1}.png'
        ...
        # Compute contrasts
            contrast_definitions = {
                'resmem': np.eye(design.shape[1])[0]
                       }           # 1st column
            ...
            for contrast_id, contrast_vector in contrast_definitions.items():
                ...
                try:
                    ...
                    beta_map_path = os.path.abspath(
                        f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/beta_maps/{sub}_task-{task}_run-{runidx + 1}_{contrast_id}_beta_map.nii.gz'
                    )
                    ...
                    plotting.plot_stat_map(
                        ...
                        display_mode='ortho',
                        cut_coords=(0, 0, 0),
                        output_file=f'/foundcog/forrestgump/foundcog-adult-2/resmem_model/glm/contrast/{sub}_task-{task}_run-{runidx + 1}_{contrast_id}_effect_size_map.png'
                    )
